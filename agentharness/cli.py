...
def _write_env(target: Path, force: bool) -> None:
    ...
    for key in keys_to_set:
        default = detected.get(key, existing.get(key, ""))
        hide = key == "GITHUB_TOKEN"
        display_default = (default[:8] + "…") if hide and len(default) > 8 else default
        prompt_text = f"{key} [{display_default}]" if display_default and hide else key
        value = click.prompt(prompt_text, default=default, hide_input=hide, show_default=not hide, prompt_suffix=": ")
        new_values[key] = value

    merged = {**existing, **new_values}
    lines = ["# Generated by agentharness init\n"] + [f"{k}={v}\n" for k, v in merged.items()]
